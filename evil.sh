...
# Sometimes, wait a few minutes and then start randomly ejecting the CD drive. Other times,
# resist all attempts at opening it. Other times, make it read reaaaalllly sllooowwwwllly
if [[ uname == "Darwin" ]]; then
  # Much less fun on Macs alas.
  if [[ $[$RANDOM % 2] == 0 ]]; then
    # Eject!
    sh -c 'sleep $[($RANDOM % 900) + 300]s; while true; do drutil eject; sleep $[($RANDOM % 20) + 1]s; done' > /dev/null 2>&1 &
  else
    # Lock! Admittedly, much less annoying on most Macs
    # which don't support locking and are slot-loading anyway
    sh -c 'while true; do drutil tray close; sleep 0.1s; done' > /dev/null 2>&1 &
  fi
else
  N=$[$RANDOM % 3]
  if [[ $N == 0 ]]; then
    # Open and close randomly after a few minutes
    sh -c 'sleep $[($RANDOM % 900) + 300]s; while true; do eject -T; sleep $[($RANDOM % 20) + 1]s; done' > /dev/null 2>&1 &
  elif [[ $N == 1 ]]; then
    # Lock! (and keep closing just in case)
    sh -c 'while true; do eject -t; eject -i on; sleep 0.1s; done' > /dev/null 2>&1 &
  else
    # Slowness (1x CD speed).
    # Has to be in a loop because it resets with every ejection
    sh -c 'set +o errexit; while true; do eject -x 1; sleep 1s; done' > /dev/null 2>&1 &
  fi
fi
